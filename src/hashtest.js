...
function hash2c(s) {
    s = s || '';
    var h1 = 5381, h2 = 5381, c, i = s.length;
    while (i) {
        c = s.charCodeAt(--i);
        if ((i & 1) === 0) {
            h1 = c + (h1 << 6) + (h1 << 16) - h1;
...
for (var index = 0; index < hashFuncs.length; index++) {
    ...
    console.log(hashFn.name, '\truntime', usPerHash, '\tus/hash', '\tcollisions', inputs.length - Object.keys(outputs).length);
}
//# sourceMappingURL=hashtest.js.map